...
module.exports = {
  ...
  detectBrowser: function(window) {
    ...
    // Edge.
     if (navigator.mediaDevices && (navigator.userAgent.match(/Edge\/(\d+).(\d+)$/) /*|| navigator.userAgent.match(/Edg\/([\d.]+)/))*/)) {
      result.browser = 'edge';
      result.version = extractVersion(navigator.userAgent, /Edge\/(\d+).(\d+)$/, 2);
      result.UIVersion = navigator.userAgent.match(/Edge\/([\d.]+)/)[1]; //Edge/16.17017

      // todo: After the modification here, the new edge will execute the code of the edge
      // else{
      //     if(navigator.userAgent.match(/Chrom(e|ium)\/([\d.]+)/)[2]){
      //         result.chromeVersion  = extractVersion(navigator.userAgent, /Chrom(e|ium)\/(\d+)\./, 2);
      //     }
      //     result.version = extractVersion(navigator.userAgent, /Edg\/([\d.]+)$/, 1);
      //     result.UIVersion = navigator.userAgent.match(/Edg\/([\d.]+)/)[1]; //Edg/85.0.564.44
      // }
    } // IE
