...
module.exports = function setup(env) {
   ...
   // Active `debug` instances.
    ...
    function sessionStorageSave() {
        env.sessionStorage.dbName = 'gerritLogs'
        ...
        var tabID = (env.sessionStorage.tabID && env.sessionStorage.closedLastTab !== '2') ? env.sessionStorage.tabID : (env.sessionStorage.tabID = 'gerritLogs');
   ...
   */
    ...
    function exportLog(feedback, dbName, day, logLevel, moduleName) {
        if(dbName){
           ...
           function(logs, time, localLogsDB, error) {
                if(logs) {
                    try {
                        ...
                        let fileName = 'gerritLog-' + hostName + '-' + getNowFormatDate() + '.log';
